<template>
  <div class="min-h-screen px-4 py-6 sm:px-6">
    <div class="max-w-3xl mx-auto w-full">
      ...
      <!-- Header -->
      <header class="flex items-center gap-3 mb-8 sm:mb-10">
        <div class="flex items-center gap-3 flex-1 min-w-0">
          <div class="w-10 h-10 sm:w-11 sm:h-11 rounded-full flex items-center justify-center text-xl font-bold shrink-0
            text-[var(--bg-primary)] bg-gradient-to-br from-gold to-gold-dim">
            五
          </div>
        ...
        <div class="flex items-center gap-2 shrink-0">
          <div
            ...
            class="flex items-center gap-2 bg-card border border-card rounded-lg px-3 py-2 text-sm cursor-pointer transition-all duration-200 hover:border-gold/50"
            @click="showUserModal = true"
          >
            <span class="text-lg">{{ gameStore.currentUser.flag }}</span>
            <span class="text-primary font-medium hidden xs:inline">{{ gameStore.currentUser.nickname }}</span>
          ...
          <button
            ...
            @click="showUserModal = true"
          >
            👤 <span class="hidden sm:inline">{{ $t('entry.start_game') }}</span>
          </button>
          <ThemeToggle />
          <LangSwitcher />
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <div
          v-for="(model, idx) in gameStore.aiModels"
          :key="model.id"
          class="relative bg-card border-2 rounded-2xl p-5 sm:p-6 cursor-pointer transition-all duration-300
            opacity-0 animate-fade-in-up hover:-translate-y-1"
          :class="selectedId === model.id
            ? 'border-gold shadow-gold-lg'
            : 'border-card hover:border-gold/40 hover:shadow-card-hover'"
          :style="{ animationDelay: `${idx * 100}ms`, animationFillMode: 'forwards' }"
          @click="selectModel(model)"
        >
          <!-- Model icon -->
          <div
            class="w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center text-2xl sm:text-3xl mb-4"
            :style="{ background: model.color }"
          >{{ model.icon }}</div>

          <!-- Info -->
          <h3 class="text-lg sm:text-xl font-display font-semibold text-primary mb-0.5">{{ model.name }}</h3>
          <p class="text-secondary text-xs sm:text-sm mb-3">{{ model.provider }}</p>
          <div class="flex gap-1">
            <span
              v-for="i in 5" :key="i"
              class="text-sm"
              :class="i <= model.difficulty ? 'text-gold' : 'text-muted'"
            >★</span>
          </div>
          ...
          <div class="flex gap-6 mt-4 pt-4 border-t border-white/10">
            <div class="flex flex-col">
              <span class="text-xl sm:text-2xl font-bold text-primary">{{ model.wins }}</span>
              <span class="text-xs text-muted uppercase tracking-wide">{{ $t('select.wins') }}</span>
            </div>
            <div class="flex flex-col">
              <span class="text-xl sm:text-2xl font-bold text-primary">{{ model.games }}</span>
          ...
          <div
            ...
            class="absolute top-4 right-4 w-7 h-7 rounded-full flex items-center justify-center bg-gold text-[var(--bg-primary)] font-bold text-sm"
          >✓</div>
        </div>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 justify-center">
        <button
          class="flex items-center justify-center gap-3 px-6 py-4 rounded-xl text-base font-semibold transition-all duration-200
        ...
        <button
          class="flex items-center justify-center gap-3 px-8 py-4 rounded-xl text-base font-semibold transition-all duration-200 btn-gold
      ...
      >
        <div class="modal-box bg-card border border-card rounded-2xl p-7 sm:p-10 w-full max-w-sm shadow-2xl">
          ...
          <div class="text-center mb-7">
            <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center text-xl sm:text-2xl font-bold mx-auto mb-3
              text-[var(--bg-primary)] bg-gradient-to-br from-gold to-gold-dim">
              五
            </div>
          ...
          <form @submit.prevent="handleSubmit" class="flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <div
                class="flex items-center gap-2.5 bg-deep border rounded-lg px-3 py-2.5 transition-colors duration-200 focus-within:border-gold"
                :class="errors.nickname ? 'border-red-600' : 'border-white/10'"
              >
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <div
                class="flex items-center gap-2.5 bg-deep border rounded-lg px-3 py-2.5 transition-colors duration-200 focus-within:border-gold"
                :class="errors.email ? 'border-red-600' : 'border-white/10'"
              >
...
import { useGameStore, type AIModel } from '~/stores/gameStore'
...
onMounted(async () => {
  ...
  if (gameStore.selectedModel) selectedId.value = gameStore.selectedModel.id
